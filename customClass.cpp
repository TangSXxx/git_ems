...
QVariant CustomSqlQueryModel::data(const QModelIndex &index, int role) const
{
    ...
    if(Qt::ForegroundRole == role && (this->headerData(index.column(),Qt::Horizontal).toString() == "网络状态")){
        if(index.data() == "在线"){
            value = QBrush(Qt::green);
        }
        else if(index.data() == "离线"){
            value = QBrush(Qt::red);
        }
        else{
            value = QBrush(Qt::yellow);
    ...
    if(Qt::ForegroundRole == role && (this->headerData(index.column(),Qt::Horizontal).toString() == "运行状态")){
        if(index.data() == "空闲"){
            value = QBrush(Qt::green);
        }
        else if(index.data() == "执行"){
            value = QBrush(Qt::red);
        }
        else{
            value = QBrush(Qt::yellow);
